    ...
    if not matched_lines[i] and notify_alias and patterns.is_aliased_notify(line, notify_alias) then
      ...
      if end_idx then
        if end_idx == i then
          ...
          local migrated, level =
            migrator.migrate_aliased_multiline(call_lines, notify_alias, levels_alias)
          if migrated then
            table.insert(matches, {
              line = i,
              end_line = end_idx,
    ...
    if not matched_lines[i] and patterns.is_existing_notify(line) then
      ...
      if end_idx then
        if end_idx == i then
          ...
          local migrated, level = migrator.migrate_existing_multiline(call_lines)
          if migrated then
            table.insert(matches, {
              line = i,
              end_line = end_idx,
